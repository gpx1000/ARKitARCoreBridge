...
void PlaneRenderer::InitializeGlContent(AAssetManager* asset_manager) {
  ...
  if (!util::LoadPngFromAssetManager(GL_TEXTURE_2D, "trigrid.png")) {
    ERROR("Could not load png texture for planes.");
  }
...
void PlaneRenderer::Draw(const glm::mat4& projection_mat,
  ...
  glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(triangles_.size()), GL_UNSIGNED_SHORT,
                 triangles_.data());
...
void PlaneRenderer::UpdateForPlane(const ArSession* ar_session,
  ...
  if (polygon_length == 0) {
    ERROR("PlaneRenderer::UpdatePlane, no valid plane polygon is found");
    return;
  }
  ...
  const int32_t vertices_length = static_cast<const int32_t>(vertices_.size());
