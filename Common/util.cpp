...
namespace util {
    ...
    ScopedArPose::ScopedArPose(const ArSession* session) {
#ifdef __ANDROID__
      ArPose_create(session, nullptr, &pose_);
#endif
    }
    ScopedArPose::~ScopedArPose() {
#ifdef __ANDROID__
        ArPose_destroy(pose_);
#endif
    }
    ArPose* ScopedArPose::GetArPose() { return pose_; }
...
static GLuint LoadShader(GLenum shader_type, const char* shader_source) {
  ...
  if (!compiled) {
    ...
    ERROR("hello_ar::util::Could not compile shader %d:\n%s\n", shader_type,
         buf);
    free(buf);
    ...
    if (link_status != GL_TRUE) {
      ...
      if (buf_length) {
        ...
        if (buf) {
          ...
          ERROR("hello_ar::util::Could not link program:\n%s\n", buf);
          free(buf);
        }
      }
...
bool LoadPngFromAssetManager(int target, const std::string& path) {
  ...
  static struct JNIData {
    ...
    ERROR("hello_ar::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  }();
...
bool LoadObjFile(AAssetManager* mgr, const std::string& file_name,
  ...
  if (asset == nullptr) {
    ERROR("Error opening asset %s", file_name.c_str());
    return false;
  }
  ...
  while (!file_string_stream.eof()) {
    ...
    if (line_header[0] == 'v' && line_header[1] == 'n') {
      ...
      int matches = sscanf(line_header, "vn %f %f %f\n", &normal[0], &normal[1],
                           &normal[2]);
      if (matches != 3) {
        ERROR("Format of 'vn float float float' required for each normal line");
        return false;
      }
      ...
      if (matches != 2) {
        ERROR("Format of 'vt float float' required for each texture uv line");
        return false;
      }
      ...
      int matches = sscanf(line_header, "v %f %f %f\n", &vertex[0], &vertex[1],
                           &vertex[2]);
      if (matches != 3) {
        ERROR("Format of 'v float float float' required for each vertice line");
        return false;
      }
  ...
  if (is_normal_available && normal_indices.size() != vertex_indices.size()) {
    ERROR("Obj normal indices does not equal to vertex indices.");
    return false;
  }

  if (is_uv_available && uv_indices.size() != vertex_indices.size()) {
    ERROR("Obj UV indices does not equal to vertex indices.");
    return false;
  }
...
void GetTransformMatrixFromAnchor(ArSession* ar_session,
  ...
  if (out_model_mat == nullptr) {
    ERROR("util::GetTransformMatrixFromAnchor model_mat is null.");
    return;
  }
